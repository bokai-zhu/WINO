...
from Phi_FEM.generate_data_Hole import call_phi_ellipse, solve_only as phi_fem_solve_only
...
model_data["lambda_loss"] = np.array([1e4, 1e-2, 1e-3, 1e-2, 0]) * 1
...
model_data["model_filename"] = "RVINO_hyperelasticity_hole_s{}_n{}_ep{}.pth".format(
...
################################################################
# Synthetic circular-hole sample: phi from call_phi_ellipse_i (center (0.65,0.65), a=b=0.2, circle), g_y=60;
# Feed into the current model, Phi-FEM reference; relative L2 only, and plot |u|_exact / |u|_pred / |u_pred-u_ref|
################################################################
_nb = model_data["grid_point_num"]
_gy_syn = 45.0
_xy_syn = np.linspace(0.0, 1.0, _nb)
_XX_syn, _YY_syn = np.meshgrid(_xy_syn, _xy_syn, indexing="xy")
_XXYY_syn = np.stack([_XX_syn.ravel(), _YY_syn.ravel()])
_params_holes_syn = np.array([[0.3, 0.3, 0.1, 0.1, 0.0]], dtype=np.float64)
_phi_syn = call_phi_ellipse(_XXYY_syn, _params_holes_syn).reshape(_nb, _nb)
_gx_syn = np.zeros((_nb, _nb), dtype=np.float64)
_gy_field_syn = np.full((_nb, _nb), _gy_syn, dtype=np.float64)
_G_syn = np.stack([_gx_syn, _gy_field_syn], axis=-1)
_Phi_syn = _phi_syn[np.newaxis, :, :, np.newaxis]
_G_batch_syn = _G_syn[np.newaxis, :, :, :]
_x_phys_syn = np.concatenate([_Phi_syn, _G_batch_syn], axis=-1)
_x_t_syn = torch.from_numpy(_x_phys_syn).to(device)
_x_enc_syn = normalizers[0].encode(_x_t_syn)
with torch.no_grad():
    _out_syn = model(_x_enc_syn)
    _phi_h_syn = _x_t_syn[:, :, :, 0:1]
    _u_h_syn = _out_syn[:, :, :, 0:2]
_t0_syn, _sol_syn = phi_fem_solve_only(
    _Phi_syn.astype(np.float64),
    _G_batch_syn.astype(np.float64),
    init_guess=None,
    nb_vert=_nb,
    deg_v=2,
)
if _sol_syn[0] is None:
    print(
        "--- synthetic sample (circular hole g_y=60): Phi-FEM did not converge, skip L2 and plotting;"
        f"t={_t0_syn} ---"
    )
else:
    _U_syn = _sol_syn[0]
    _u_ref_np_syn = _U_syn[0]
    _u_ref_t_syn = torch.from_numpy(_u_ref_np_syn).to(device, dtype=_u_h_syn.dtype)
    _mask_syn = get_node_masks(_phi_h_syn).squeeze(-1).squeeze(0)
    _u_true_m_syn = _u_ref_t_syn[_mask_syn]
    _u_pred_m_syn = _u_h_syn.squeeze(0)[_mask_syn]
    _rel_l2_syn = myLoss(
        _u_pred_m_syn.reshape(1, -1), _u_true_m_syn.reshape(1, -1)
    ).item()
    print(
        "--- synthetic sample: circle (0.65,0.65), a=b=0.2, g_y=60 | "
        f"Phi-FEM time(s)={_t0_syn:.4f} | rel_L2={_rel_l2_syn:.6e} ---"
    )
    _up_np = _u_h_syn.squeeze(0).detach().cpu().numpy()
    _mag_exact = np.sqrt(_u_ref_np_syn[..., 0] ** 2 + _u_ref_np_syn[..., 1] ** 2)
    _mag_pred = np.sqrt(_up_np[..., 0] ** 2 + _up_np[..., 1] ** 2)
    _mag_err = np.sqrt(
        (_up_np[..., 0] - _u_ref_np_syn[..., 0]) ** 2
        + (_up_np[..., 1] - _u_ref_np_syn[..., 1]) ** 2
    )
    ...
    print(
        f"--- synthetic sample displacement-magnitude figures (smooth phi mask) saved: {folder}/Hole_synthetic_mag_exact.png, "
        f"mag_pred, mag_error ---"
    )
...
with torch.no_grad():
    for x, u_true in test_loader:
        ...
        g_h = x[:, :, :, 1:]
        u_h = out[:, :, :, 0:2]
        y_h = out[:, :, :, 2:6] * model_data["norm_y"]
        p_h = out[:, :, :, 6:8] * model_data["norm_p"]
        u_true1 = u_true[:, :, :, 0:2]
        ...
        mask_node_cut = get_node_mask_from_cell_mask(mask_cut_cell).to(out.dtype)
        init_guess_masked = out.clone()
        init_guess_masked[:, :, :, 0:2] = init_guess_masked[:, :, :, 0:2] * mask_node_u
        init_guess_masked[:, :, :, 2:8] = init_guess_masked[:, :, :, 2:8] * mask_node_cut
        ...
        t0, _ = phi_fem_solve_only(
            phi_i, g_i, init_guess=None, nb_vert=model_data["grid_point_num"]
        )
        gmres_it0 = int(
            getattr(
                phi_fem_solve_only,
        ...
        t1, sol1 = phi_fem_solve_only(
            phi_i, g_i, init_guess=init_i[...], nb_vert=model_data["grid_point_num"], n_load_steps_f=1
        )
        gmres_it1 = int(
            getattr(
                phi_fem_solve_only,
...
os.makedirs(folder, exist_ok=True)
nw_png = "Hole_NewtonResidual.png"
fig_nw.savefig(os.path.join(folder, nw_png), dpi=150)
plt.close(fig_nw)
print(
...
box_png = "Hole_GMRESIters_boxplot.png"
fig_box.savefig(os.path.join(folder, box_png), dpi=150)
plt.close(fig_box)
