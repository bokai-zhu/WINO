...
model_data["lambda_loss"] = np.array([1e4, 1e1, 1e-3, 1e-1]) * 1e-2
model_data["lambda_vin"] = 0.5
model_data["sigma_N"] = 0.1
...
model_data["model_filename"] = "RVINO_Gh{}_hyperelasticity_hole_s{}_n{}_ep{}.pth".format(
    model_data["sigma_N"],
...
#                              lr=model_data["learning_rate_adam"])
# scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
#     optimizer,
#     patience=model_data["patience"],
#     factor=model_data["gamma"],
#     cooldown=model_data["cooldown"],
#     min_lr=model_data["min_lr"],
# )
...
#     optimizer,
#     num_warmup_steps=20,  # give SOAP 20 steps to build a robust preconditioner
#     num_training_steps=model_data["num_epoch_adam"]
# )
# scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.999)
scheduler = None
myLoss = LpLoss(d=1, size_average=False)
t1 = default_timer()
train_l2_log, test_l2_log = train_rvino_Gh_hyperelasticity_Guass(
...
_l2_log_npz = os.path.join(
    result_dir,
    model_data["model_filename"].replace(".pth", "_l2_log.npz"),
)
np.savez_compressed(
    _l2_log_npz,
...
print(f"L2 log saved to: {_l2_log_npz}")
...
plt.xlabel(r'Epoch')
plt.savefig( folder + '/Hole_TrainingTrend.png', dpi=300)
plt.show()
...
plot_field_2d(u_exact[0, :, :, 0], 1, 1, "u Exact Solution", 
              mask=mask1, folder=folder, file='u_exact0')
plot_field_2d(u_pred[0, :, :, 0], 1, 1, "u Predicted Solution", 
              mask=mask1, folder=folder, file='u_pred0')
plot_field_2d(abs(u_exact - u_pred)[0, :, :, 0], 1, 1, "u Absolute Error", 
              mask=mask1, folder=folder, file='u_err0')

plot_field_2d(u_exact[0, :, :, 1], 1, 1, "v Exact Solution", 
              mask=mask1, folder=folder, file='v_exact0')
plot_field_2d(u_pred[0, :, :, 1], 1, 1, "v Predicted Solution", 
              mask=mask1, folder=folder, file='v_pred0')
plot_field_2d(abs(u_exact - u_pred)[0, :, :, 1], 1, 1, "v Absolute Error", 
