...
model_data["lambda_loss"] = np.array([1e3, 1e-4, 1e1, 1e3]) * 1  # 3, -3, 1, 3
model_data["lambda_Gh"] = 1
...
model_data["model_filename"] = "RVINO_hyperelasticityTrapezoid_y{}x{}_n{}_ep{}.pth".format(
...
t1 = default_timer()
train_l2_log, test_l2_log, loss_v_log, loss_y_log, loss_p_log, loss_dy_log = train_rvino_hyperelasticity_trapezoid_Guass(
...
_loss_log_npz = os.path.join(
    result_dir,
    model_data["model_filename"].replace(".pth", "_loss_log.npz"),
)
np.savez_compressed(
    _loss_log_npz,
    train_l2_log=np.asarray(train_l2_log, dtype=np.float64),
    test_l2_log=np.asarray(test_l2_log, dtype=np.float64),
    loss_v_log=np.asarray(loss_v_log, dtype=np.float64),
    loss_y_log=np.asarray(loss_y_log, dtype=np.float64),
    loss_p_log=np.asarray(loss_p_log, dtype=np.float64),
    loss_dy_log=np.asarray(loss_dy_log, dtype=np.float64),
)
print(f"Loss log saved to: {_loss_log_npz}")
...
plt.legend()
plt.savefig( folder + '/Trapezoid_TrainingTrend.png', dpi=300, bbox_inches='tight')
plt.show()
...
# mask1 = ~(get_asymmetric_node_mask(phi_grid))
...
plot_field_2d(u_exact[0, :, :, 0], 4, 2, "u Exact Solution", 
              mask=mask1, folder=folder, file='u_exact0')
plot_field_2d(u_pred[0, :, :, 0], 4, 2, "u Predicted Solution", 
              mask=mask1, folder=folder, file='u_pred0')
plot_field_2d(abs(u_exact - u_pred)[0, :, :, 0], 4, 2, "u Absolute Error", 
              mask=mask1, folder=folder, file='u_err0')

plot_field_2d(u_exact[0, :, :, 1], 4, 2, "v Exact Solution", 
              mask=mask1, folder=folder, file='v_exact0')
plot_field_2d(u_pred[0, :, :, 1], 4, 2, "v Predicted Solution", 
              mask=mask1, folder=folder, file='v_pred0')
plot_field_2d(abs(u_exact - u_pred)[0, :, :, 1], 4, 2, "v Absolute Error", 
              mask=mask1, folder=folder, file='v_err0')
