...
from Phi_FEM.generate_data_Trapezoid import compute_errors
...
model_data["grid_point_num_test"] = 51
...
print(
    f"train set npz grid: {model_data['grid_point_num']}×{model_data['grid_point_num']}; "
    f"test set npz grid: H×W={_test_h}×{_test_w} "
    f"(grid_point_num_test={model_data['grid_point_num_test']}); "
    f"compute_errors nb_cell={model_data['grid_point_num_test'] - 1}"
)
if _test_h != model_data["grid_point_num_test"] or _test_w != model_data["grid_point_num_test"]:
    print(
        f"warning: grid_point_num_test={model_data['grid_point_num_test']} "
        f"does not match test npz shape {_test_h}×{_test_w}; check npz or config."
    )
...
if plot_fig == True:
    ...
    err_bar_path = os.path.join(folder, "Trapezoid_error_bars.png")
    fig_err.savefig(err_bar_path, dpi=400, bbox_inches="tight")
    plt.close(fig_err)
    ...
    g_np = g_hw.numpy()
    _, W, _ = g_np.shape
    y_coord = np.linspace(0.0, 2.0, W)
    y_mask = (y_coord >= 1.4) & (y_coord <= 2.0)

    plt.figure()
    plt.plot(y_coord[y_mask], g_np[y_mask, -1, 1], label=r"$g_y$")
    plt.xlabel("y")
    plt.ylabel("Traction")
    plt.title("Input", fontsize=20)
    plt.savefig(
        os.path.join(folder, "Trapezoid_Input.png"),
        dpi=400,
        bbox_inches="tight",
    )
    plt.close()
    ...
    plot_u_matrix_displaced_custom_domain(
        ...
        scale=1.0,
        folder=folder,
        file="Trapezoid_Displaced_predict",
        upsample=10,
        phi=phi_np,
        x_domain=(0.0, 4.0),
        y_domain=(0.0, 2.0),
    ...
    for ig, gv in enumerate(g_sweep_values):
        plot_u_matrix_displaced_custom_domain(
            ...
            scale=1.0,
            folder=folder,
            file=f"Trapezoid_Displaced_predict_g{gv}",
            upsample=10,
            phi=phi_np,
            x_domain=(0.0, 4.0),
            y_domain=(0.0, 2.0),
