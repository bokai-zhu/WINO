...
def call_psi(xy, params_gauss):
    """psi(x,y)=sum_k exp(-(x-xk)^2/(2*sigma_k)-(y-yk)^2/(2*gamma_k))."""
    psi = 0.0
    for k in range(params_gauss.shape[0]):
        xk, yk, sigma_k, gamma_k = params_gauss[k]
        psi += np.exp(
            -((xy[0] - xk) ** 2) / (2.0 * sigma_k)
            - ((xy[1] - yk) ** 2) / (2.0 * gamma_k)
        )
    return psi

def call_phi(xy, params_gauss, psi_max):
    """phi(x,y) = -psi(x,y) + 0.5 * max_{[0,1]^2} psi(x,y)."""
    return -call_psi(xy, params_gauss) + 0.5 * psi_max
...
def create_FG_numpy(nb_data, nb_vert):
    ...
    # Sample the two components of F independently (same expression)
    ...
    amplitude_u = np.random.uniform(3, 6, size=[nb_data, 1]) * np.random.choice(
        [-1, 1], size=[nb_data, 1]
    )
    ...
    amplitude_v = np.random.uniform(3, 6, size=[nb_data, 1]) * np.random.choice(
        [-1, 1], size=[nb_data, 1]
    )
    ...
    # 4.2 random complex shapes: superpose 3 anisotropic Gaussians to build psi, then map to phi
    # For smoother geometry: enlarge sigma/gamma and keep centers in the middle to avoid spikes and isolated islands
    sampler = qmc.LatinHypercube(d=4 * N_GAUSS)
    sample = sampler.random(nb_data)
    low_bounds = [
        0.4, 0.55, 0.04, 0.04,  # near the top vertex
        0.3, 0.3, 0.04, 0.04,  # near the lower-left corner
        0.55, 0.3, 0.04, 0.04,  # near the lower-right corner
    ]
    up_bounds = [
        0.6, 0.7, 0.05, 0.05,
        0.45, 0.45, 0.05, 0.05,
    ...
    gauss_flat = qmc.scale(sample, low_bounds, up_bounds)
    ...
    for n in range(nb_data):
        params_gauss = gauss_flat[n].reshape(N_GAUSS, 4)
        psi_grid = call_psi(XXYY, params_gauss)
        psi_max = float(np.max(psi_grid))
        psi_max_arr.append(psi_max)
        phi_n = call_phi(XXYY, params_gauss, psi_max).reshape((nb_vert, nb_vert))
        Phi.append(phi_n)
    phi = np.array(Phi)
    ...
    psi_max_arr = np.array(psi_max_arr, dtype=np.float64).reshape(nb_data, 1)
...
class Phi:
    def __init__(self, params_gauss, psi_max) -> None:
        self.params_gauss = np.asarray(params_gauss).reshape(N_GAUSS, 4)
        self.psi_max = float(psi_max)

    def eval(self, x):
        return call_phi(x, self.params_gauss, self.psi_max)
...
class PhiFemSolver:
    ...
    def solve_one(self, i):
        ...
        if len(boundary_cells_0) > 0:
            ...
            # ds(4): facets of boundary cells that lie on submesh ∂Ω (arbitrary boundary and outer frame)
    ...
    if save:
        ...
        if n_train is not None and n_test is not None:
            ...
            path_train = os.path.join(
                data_dir,
                f"Hyperelasticity_Arbit_GF_u_s{nb_vert}_n{n_tr}_train.npz",
            )
            path_test = os.path.join(
                data_dir,
                f"Hyperelasticity_Arbit_GF_u_s{nb_vert}_n{n_te}_test.npz",
            )
            np.savez_compressed(
                path_train,
